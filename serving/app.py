...
imputer = utils.load_transformers(
    os.path.join(config["encoders_dir"], "imputer.pkl"))
scaler = utils.load_transformers(
    os.path.join(config["encoders_dir"], "scaler.pkl"))
ohencoders = []
for col in config["categorical_features"]:
    ohencoders.append(utils.load_transformers(
        os.path.join(config["encoders_dir"], col+"_encoder.pkl")))
...
def predict():
    ...
    print(req)
    df = pd.DataFrame([req], columns=["x1", "x2",
                      "x3", "x4", "x5", "x6", "x7"])
